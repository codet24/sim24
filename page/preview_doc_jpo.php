...
$id = $_GET['id']; 
$tampil=mysqli_fetch_array(mysqli_query($con,"SELECT a.*,b.*,c.*, a.id as pengajuan_id, a.nama_pekerjaan, 
                    b.unit, b.harga_per_unit, sum(b.unit * b.harga_per_unit) as total FROM 
                    tbl_doc_pengajuan a LEFT JOIN tbl_doc_pengajuan_detail b ON 
                    ...
                    a.id_karyawan_yang_mengetahui=c.id AND  a.id_karyawan_finance=c.id where a.deleted=0 and a.id='$id'
...
$pdf->Cell(90,7,'FORM PENGAJUAN JOB PURCHASE ORDER',0,1,'C');
$pdf->Cell(60);
$pdf->SetFont('Arial','B',13);
...
$pdf->Cell(5,6,':',0,0);
$pdf->Cell(80,6,$tampil['nama_pekerjaan'],0,0);
...
$query=mysqli_query($con,"SELECT a.*,b.*,c.*, a.id as pengajuan_id, a.nama_pekerjaan, 
                    b.unit, b.harga_per_unit, b.unit * b.harga_per_unit as subtotal FROM 
